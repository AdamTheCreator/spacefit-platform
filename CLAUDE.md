...
**Current prod status (2026-07-09):** `render.yaml` is back on `LLM_PROVIDER=anthropic` (issue #63; the Baseten config is preserved as comments for when Mac Mini/Ollama serving is ready). The platform default model is **`claude-haiku-4-5`** — the cheapest current Claude model ($1/$5 per MTok) — via `anthropic_model` in `app/core/config.py`, and it serves all non-BYOK tiers (no `GOOGLE_GEMINI_API_KEY` is set in Render, so the free-tier Gemini split falls through to Haiku too). `ANTHROPIC_API_KEY` must be set in the Render dashboard (`sync: false`) or chat calls fail. The specialist `MODEL_TIER_MAP` (fast/balanced/deep) also resolves to `claude-haiku-4-5` platform-wide; BYOK per-specialist overrides remain the path to stronger models.
