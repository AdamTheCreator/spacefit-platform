...
Active effort to move the chat off the Anthropic API onto a self-hosted, fine-tuned open model. **`MIGRATION.md` (repo root) is the running log + decision record (D1–D21) — read it first.** Status: baseline + evals ✅, model selected & L4-validated ✅, advisory-failure diagnosis ✅, data curation ✅, **and the advisory LoRA has now finished training on Together AI ✅** (job `ft-0a9cfffd-ba02` → adapter `adamthecreator/Qwen2.5-7B-Instruct-spacegoose-advisor-a9cac113`, 4 epochs, lr 1e-4, ~20k tokens). **Next:** serve the adapter on a Baseten L4 (vLLM `--enable-lora`) and score it on the held-out advisory eval against base Qwen (2.80) and Haiku (4.35).
