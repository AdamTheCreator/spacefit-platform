...
Active effort to move the chat off the Anthropic API onto a self-hosted, fine-tuned open model. **`MIGRATION.md` (repo root) is the running log + decision record (D1–D21) — read it first.** Status: baseline + evals ✅, model selected & L4-validated ✅, data curation ✅, and **two LoRA rounds trained/served/judged (§4.10–§4.11): v1 (Together's unpinned r=64 defaults) = 2.50/5, v2 (r=16/α=32 + sectioned-format augmentation) = 2.35/5, vs base Qwen 2.80 / Haiku 4.35 — all on the same re-anchored judge.** The three-run trend is the finding: tone rises (2.25→3.00→3.25) while grounding falls (3.00→2.25→1.75); v2 even flips GO↔PASS on identical facts depending on input phrasing. **Amended diagnosis: 29–44 human-gold pairs teach the memo voice/register but cannot teach the input→disposition mapping.** Both adapters + base co-serve on the one L4 (`--enable-lora`, two `--lora-modules` entries; adapters in the git-ignored Truss `data/`). **Next: Phase 3b teacher augmentation** (pre-planned in §4.9): Claude few-shot-anchored on the real memos writes ~100–200 grounded GO/PASS pairs over diverse void/synthetic inputs; human memos stay the voice anchor. Fallbacks if 3b underdelivers: mix general instruct data; D18 14B.
