...
alembic upgrade head                                  # apply migrations (latest is 037)
...
## Model migration: self-hosted inference on Baseten (in progress)

Active effort to move the chat off the Anthropic API onto self-hosted, fine-tuned open models on Baseten. **`MIGRATION.md` (repo root) is the running log + decision record (D1–D11) — read it first.** Status: Phase 0 (baseline + evals) ✅ and Phase 1 (model selection + L4 validation) ✅; Phase 2 (fine-tuning data prep) is next.

- **Why it's tractable:** every model call already goes through `app/llm/`, which speaks both `anthropic` and `openai_compatible`. Baseten serves an OpenAI-compatible API, so the seam already exists — point the `openai_compatible` provider at a Baseten endpoint and the whole chat path works.
- **Eval harness — `backend/evals/`** (deliberately NOT in the CI unit-test gate; it hits live models). Two dimensions, both provider-agnostic via `EVAL_*` env vars so the same suite scores Anthropic / Gemini / HF / Baseten on equal footing:
  - `run_eval.py` — tool-calling + routing accuracy. Grading in `grade.py` (pure stdlib; offline self-check: `python evals/grade.py`). 90% tool-call bar; cases in `cases/*.jsonl`. `tests/test_evals.py` unit-tests the grader.
  - `run_advisory.py` — **advisory quality** (LLM-as-judge in `judge.py`): does the model reason about property→client-fit and give a clear go/no-go? This is the product value, not tool-picking, and it's what the void/memo fine-tune targets.
...
- **Candidate model: `Qwen/Qwen2.5-7B-Instruct`** (Apache-2.0, fits a 24 GB L4, strong tool caller). Deployed via a config-only Truss at `backend/baseten/qwen25-7b-instruct/` (vLLM image, `--tool-call-parser hermes`, scale-to-zero). Benchmark vs the Haiku baseline: tool-calling **92.9%** (beats Haiku, clears the bar) + ~0.9 s warm TTFT, **but advisory quality 2.80/5 vs 4.35 → do not ship base Qwen; closing the advisory gap is the fine-tune's job.**
- **Gotchas:** the HF serverless router is unreliable for tool-calling (heterogeneous providers 400 on `tool_choice`) — use controlled serving. The configured `anthropic_model` default `claude-3-5-haiku-20241022` is **retired (404s)**; fix-forward to `claude-haiku-4-5` is owed (see MIGRATION.md §4.1). Vision document parsing stays on Anthropic (out of scope). BYOK stays as the "bring your own frontier model" path; Baseten becomes the platform default.
