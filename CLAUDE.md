...
Active effort to move the chat off the Anthropic API onto a self-hosted, fine-tuned open model. **`MIGRATION.md` (repo root) is the running log + decision record (D1–D21) — read it first.** Status: baseline + evals ✅, model selected & L4-validated ✅, advisory-failure diagnosis ✅, data curation ✅, **v1 LoRA trained (Together, `ft-0a9cfffd-ba02`), served next to base Qwen on the L4, and judged (MIGRATION.md §4.10): 2.50/5 off-format vs base 2.80 / Haiku 4.35 — the broker voice transferred (strong in-format held-out memo) but the input surface overfit** (Together's unpinned r=64/α=128 LoRA defaults + a single training input shape). **Next: v2** — `together_submit.py` now pins r=16/α=32, and `finetune/diversify_formats.py` augments the train set with sectioned `## Property/## Data` input renderings so the eval/product shape is in-distribution; retrain, re-judge on the same yardstick.
