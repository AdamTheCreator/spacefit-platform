...
5. **Failure semantics (the stuck-chip contract)** — a bad *plan* degrades safely (parse/LLM failures fall back to `["scout"]`), but an exception inside a specialist *leg* (stream, tool dispatch, DB save) does NOT re-run the legacy path: it's caught in the routing branch, the user gets an error bubble, and — critically — `_run_specialist_routing_turn` first **sweeps every workflow step that never reached a terminal status to `"error"`** before re-raising. Without the sweep, the `AgentActivityPanel` chip for the in-flight specialist spins forever while the turn looks finished. The frontend treats `error` / `timed_out` / `circuit_open` as terminal when releasing the activity spinner, and `useChat`'s `ws.onclose` settles any still-`pending`/`running` chips to `error` (a dropped socket orphans the turn server-side — its updates can never arrive on the reconnected socket). `test_specialist_routing.py::test_specialist_failure_sweeps_unfinished_steps_to_error` enforces the sweep; any new code path that emits `workflow_init` steps must guarantee a terminal `workflow_update` for each on every exit, including exceptions.
...
- **Outreach, end-to-end.** Campaign send (`app/services/email_blast.py::send_email`) picks a transport in priority order: the user's connected **Gmail** (`get_user_gmail_tokens` in `app/services/gmail.py`, tokens from the google `OAuthAccount`) → **SMTP** → **dev-log**. Open/click **tracking** (`OutreachRecipient.tracking_id` + pixel/link wrapping in `app/api/tracking.py`) is applied *before* transport selection, so it works on every path; set `API_BASE_URL` in prod so the links resolve. **Reply triage:** `app/services/outreach_replies.py::sync_replies_for_user` (Gmail-backed, best-effort; no-op without Gmail) + `GET /outreach/threads` + `POST /outreach/sync-replies` (migration `036` added `reply_*` fields to `outreach_recipients`); the Dashboard reply rows read `/outreach/threads`. The Contacts directory's "Send to Outreach" pre-fills the existing `EmailComposer` via its `initialRecipients` prop (passed through router state). **Address autocomplete:** the composer's "What's this about?" field uses `components/ui/AddressAutocomplete.tsx`, backed by the authenticated proxy `GET /api/v1/places/autocomplete` (`app/api/places.py`, reuses the server-side `google_places_api_key`). **Never call Google's Places JSON APIs from the browser** — they don't serve CORS headers and it would ship the key in the bundle; add another proxy endpoint instead (a `/places/details` proxy is the intended path if a consumer ever needs lat/lng). The component degrades to a plain input when the backend reports `enabled: false` (no key configured), and its `onlyWhenAddressLike` prop gates suggestions on address-shaped text (leading street number or street-suffix word) so prose-friendly fields don't get popups mid-sentence — pass it whenever the field isn't strictly an address.
