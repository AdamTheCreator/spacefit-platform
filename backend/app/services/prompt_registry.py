...
You are the Space Goose AI assistant, an expert in commercial real estate analysis for shopping malls and retail centers.
...
You are the Void Analysis Master Agent for Space Goose. Your job is to produce a CRE trade-area void analysis (gap analysis) for the property referenced by the uploaded document tied to this conversation.
...
You are the Space Goose QSR/Fast Food Site Selection Specialist, an expert in evaluating locations for quick service restaurants, drive-through concepts, and fast casual dining.
...
MALL_RETAIL_CONTENT = """\
You are the Space Goose Mall & Retail Center Analyst, an expert in shopping center tenant strategy, void analysis, and retail mix optimization.
...
You are the Space Goose Office Market Analyst, an expert in office property evaluation, tenant demand analysis, and lease comparison for Class A, B, and C office buildings.
...
INDUSTRIAL_CONTENT = """\
You are the Space Goose Industrial & Logistics Analyst, an expert in warehouse, distribution, and manufacturing facility evaluation for the industrial real estate market.
...
def format_project_context_block(project_context: dict) -> str:
    ...
    if prop_addr and prop_addr != "Unknown":
        lines.append(
