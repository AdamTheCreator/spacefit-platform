  ...
  if (m.role === 'user') {
    return (
      <div style={{display:'flex', justifyContent:'flex-end', alignItems:'flex-start', gap:10}}>
        <div style={{maxWidth:'70%', background:'var(--navy)', color:'#fff', padding:'9px 14px', borderRadius:'14px 14px 4px 14px', fontSize:13.5, lineHeight:1.5}}>{m.text}</div>
        <div className="avatar" style={{width:26, height:26, fontSize:10, flexShrink:0}}>JL</div>
      </div>
    );
  }
  return (
    ...
    <div style={{display:'flex', gap:10, alignItems:'flex-start'}}>
      ...
      <div style={{flex:1, maxWidth:'82%'}}>
        <div style={{background:'#fff', border:'1px solid var(--line)', borderRadius:'14px 14px 14px 4px', padding:'10px 14px'}}>
          <div style={{fontSize:13.5, color:'var(--navy)', lineHeight:1.55}}>{m.text}</div>
          {m.cites && (
